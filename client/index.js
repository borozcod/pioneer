...
const bas = require('./js/bas.js');
const TextGeometry = require('./js/TextGeometry.js');
const FontUtils = require('./js/pnltri.min.js');
const droid_sans = require('./js/droid_sans_bold.typeface.js');
...
function init() {
  ...
  var pioneerAnimation = createTextAnimation('PIONEER');
  var textAnimation = createTextAnimation('X');
  var worshipAnimation = createTextAnimation('WORSHIP');  
  pioneerAnimation.position.y = 50;
  textAnimation.position.y = 0;
  worshipAnimation.position.y = -50;
  root.scene.add(textAnimation);
  root.scene.add(pioneerAnimation);
  root.scene.add(worshipAnimation);
  ...
  window.addEventListener('mousedown', function(e) {
    tl.fromTo(textAnimation, 2,
      ...
      {animationProgress:1.0, ease:Power1.easeInOut, onComplete:function() {
        root.scene.remove(textAnimation);
      }},
      0
    ).fromTo(pioneerAnimation, 2,
      {animationProgress:0.0},
      {animationProgress:1.0, ease:Power1.easeInOut, onComplete:function() {
        root.scene.remove(pioneerAnimation);
      }},
      0.25
    ).fromTo(worshipAnimation, 2,
      {animationProgress:0.0},
      {animationProgress:1.0, ease:Power1.easeInOut, onComplete:function() {
        root.scene.remove(worshipAnimation);
      }},
      0.5
    ).to("#time-loc", 0.25, { opacity:1});
    e.preventDefault();
  });
